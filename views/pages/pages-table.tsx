...
export default function PagesTable() {
  ...
  const getPagesList = async (limit: number, page: number) => {
    try {
      setLoading(true);
      const res = await axios.get(`/api/pages?limit=${limit}&page=${page}`);
        ...
        <div className="space-x-2">
          <Tooltip>
            <TooltipTrigger asChild>
              <Link href={"/admin/pages/" + row.original.slug}>
                <Button
                  variant="outline"
                  size="icon"
